!function(){
  ...
  var ajaxCommit = function(){
  	  var index = sType.selectedIndex,
  	  	   ...
  	  	   request.open("GET", '/v2/h5/content/recommend.html?' + 'type=' + value + '&content=' + conVal + '&contact=' + mobileVal);
  	  	   // request.open("GET", 'yan.txt?'+ 'type=' + value + '&content=' + conVal + '&contact=' + mobileVal);
  ...
  if (submitBtn) {
  		H.Event.tapClick(function() { //保存按钮点击    
  		    var flag =  checkForm();
  		    if(flag){        
  		      ajaxCommit();
  		      console.log('yes');
