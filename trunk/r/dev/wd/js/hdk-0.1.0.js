		...
		var completed = function () {
			...
			setTimeout(function () {
				...
				if (modLoaded) {
					...
					hdk.status = 'MOD_NOT_LOADED';
					hdk.log.info("HDK initialized with error. status=" + this.status);
				}
				callback(hdk.mode);
			}, 0);
		};
...
hdk.module('event', {
	...
	init: function () {
		...
		hdk.event.defaultEventHandlers['backButtonClicked'] = function () { window.history.back();}
	},
...
hdk.module('user', {
	...
	init: function () {
		...
		hdk.config.webApi.pathLogin = '/widget/html/login.html';
		hdk.config.webApi.pathRegister = '/widget/html/register.html';
		hdk.config.webApi.pathResetPassword = '/widget/html/retrievePasswd.html';
		...
		if (hdk.mode == 'native') { // 根据原生信息判断是否登录
			...
			if (hdk.loginData && hdk.loginData != "") {
				...
				Cookies.set(hdk.user.info.sessionKey, hdk.user.info.sessionValue,
					{expires: 30, path: '/'});
				Cookies.set('hdk_user', JSON.stringify(hdk.loginData), {expires: 30, path: '/'});
			} else {
	...
	auth: function (callback) {
		...
		deferred.always(function (status, data) {
			...
			if (status === 'SUCCESS') {
				if (data && data.sessionKey && data.sessionValue) { //登录成功
					Cookies.set(data.sessionKey, data.sessionValue, {expires: 30, path: '/'});
					Cookies.set('hdk_user', JSON.stringify(data), {expires: 30, path: '/'});
		...
		if (hdk.support) {
			...
			window.location.href
				= hdk.config.webApi.host
				+ hdk.config.webApi.pathLogin
	...
	register: function (callback) {
		...
		if (hdk.support) {
			...
			window.location.href
				= hdk.config.webApi.host
				+ hdk.config.webApi.pathRegister
				+ '?from=' + window.location.href;
		}
	}
	,
	...
	resetPassword: function (callback) {
		...
		if (hdk.support) {
			...
			window.location.href
				= hdk.config.webApi.host
				+ hdk.config.webApi.pathResetPassword
				+ '?from=' + window.location.href;
		}
	}
	,
